...
import { rewriteDefault, SFCBlock, SFCDescriptor } from '@nkduy/compiler-sfc'
...
export async function transformMain(
  ...
  if (devServer && !ssr && !isProduction) {
    ...
    output.push(
      `__KDU_HMR_RUNTIME__.createRecord(_sfc_main.__hmrId, _sfc_main)`
    )
    ...
    output.push(
      ...
      `  if (_rerender_only) {`,
      `    __KDU_HMR_RUNTIME__.rerender(updated.__hmrId, updated.render)`,
      `  } else {`,
      `    __KDU_HMR_RUNTIME__.reload(updated.__hmrId, updated)`,
      `  }`,
      `})`
    )
  }
...
async function genTemplateCode(
  ...
  if (!template.lang && !template.src) {
    ...
    const query = `?kdu&type=template${srcQuery}${attrsQuery}`
...
async function genScriptCode(
  ...
  if (script) {
    ...
    if (
      ...
      const query = `?kdu&type=script${srcQuery}${attrsQuery}`
      const request = JSON.stringify(src + query)
      scriptCode =
...
async function genStyleCode(
  ...
  if (descriptor.styles.length) {
    for (let i = 0; i < descriptor.styles.length; i++) {
      ...
      const query = `?kdu&type=style&index=${i}${srcQuery}`
...
async function genCustomBlockCode(
  ...
  for (let index = 0; index < descriptor.customBlocks.length; index++) {
    ...
    const query = `?kdu&type=${block.type}&index=${index}${srcQuery}${attrsQuery}`
    const request = JSON.stringify(src + query)
